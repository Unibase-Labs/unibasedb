...
def test_inmemory_unibase_batch(docs_to_index, replicas, shards, protocol, tmpdir):
    query = docs_to_index[:10]
    port = random_port()
    with InMemoryExactNNUnibase[MyDoc](workspace=str(tmpdir)).serve(replicas=replicas, shards=shards, port=port,
...
@pytest.mark.parametrize('replicas', [1, 3])
@pytest.mark.parametrize('protocol', ['grpc', 'http', 'websocket'])
def test_inmemory_unibase_single_query(docs_to_index, limit, replicas, shards, protocol, tmpdir):
    query = docs_to_index[100]
    port = random_port()
    with InMemoryExactNNUnibase[MyDoc](workspace=str(tmpdir)).serve(workspace=str(tmpdir), replicas=replicas, shards=shards, port=port,
                                              protocol=protocol, timeout_ready=10000) as db:
        db.index(inputs=docs_to_index)
        if replicas > 1:
            time.sleep(2)
...
@pytest.mark.parametrize('replicas', [1, 3])
@pytest.mark.parametrize('protocol', ['grpc', 'http', 'websocket'])
def test_inmemory_unibase_delete(docs_to_index, replicas, shards, protocol, tmpdir):
    query = docs_to_index[0]
    port = random_port()
    delete = MyDoc(id=query.id, text='', embedding=np.random.rand(128))
    with InMemoryExactNNUnibase[MyDoc]().serve(workspace=str(tmpdir), replicas=replicas, shards=shards, port=port,
                                              protocol=protocol, timeout_ready=10000) as db:
        db.index(inputs=docs_to_index)
        if replicas > 1:
            time.sleep(2)
...
@pytest.mark.parametrize('replicas', [1, 3])
@pytest.mark.parametrize('protocol', ['grpc', 'http', 'websocket'])
def test_inmemory_unibase_udpate_text(docs_to_index, replicas, shards, protocol, tmpdir):
    query = docs_to_index[0]
    port = random_port()
...
@pytest.mark.parametrize('replicas', [1, 3])
@pytest.mark.parametrize('protocol', ['grpc', 'http', 'websocket'])
def test_inmemory_unibase_restore(docs_to_index, replicas, shards, protocol, tmpdir):
    query = docs_to_index[:100]
    port = random_port()
